...
save_dir = os.path.join("F:\\paper_code\\wofost", "MCMC", tabName)
isdir_demo(save_dir)
...
def get_LAI(table_name):
    ...
    obs_LAI = pd.read_csv(os.path.join(lai_dir, f"{table_name}{n_amount[0]}Y2021_LAI_OBS.csv"))

    obs_LAI.index = pd.to_datetime(obs_LAI.day)
    obs_LAI.drop("day", axis=1, inplace=True)
    obs_LAI1 = pd.read_csv(os.path.join(lai_dir, f"{table_name}{n_amount[1]}Y2021_LAI_OBS.csv"))
    obs_LAI1.index = pd.to_datetime(obs_LAI1.day)
    obs_LAI1.drop("day", axis=1, inplace=True)

    obs_data = [
       [obs_LAI, obs_LAI1],
        [yield_dict[f"{table_name}{n_amount[0]}"][0], yield_dict[f"{table_name}{n_amount[1]}"][0]]
    ]
    return obs_data
...
sited = WOFOST80SiteDataProvider(
    WAV=18, NAVAILI=20.5, PAVAILI=50.0, KAVAILI=250.0)
global parameters
...
agro_list = [my_agro(yaml_agro_2021, n_amount[0]), my_agro(yaml_agro_2021, n_amount[1])]
